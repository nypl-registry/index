...
var LocParser = require('./js-loc-callnumbers/loader').LocParser
...
// e.g. pad(10, 4, '-'); // --10
exports.lpad = function (n, width, z) {
  z = z || '0'
  n = n + ''
  return n.length >= width ? n : new Array(width - n.length + 1).join(z) + n
}

exports.parseUrn = function (urn) {
  var parts = urn.split(':')
  if (parts.length >= 3 && parts[0] === 'urn') parts.shift()
...
exports.extractIndexFields = function (agentObject) {
  ...
  if (flat.objectUri['rdf:type']) {
    ...
    results['type'] = (typeof flat.objectUri['rdf:type']) === 'object' ? flat.objectUri['rdf:type'][0] : flat.objectUri['rdf:type']
  ...
  if (flat.objectUri['dcterms:identifier']) {
    ...
    // Save idLcc & idLccSort:
    flat.objectUri['dcterms:identifier'].filter((urn) => urn.match(/^urn:lcc:/)).forEach((urn) => {
      var num = exports.parseUrn(urn).id
      results.idLcc = num
      results.idLccSort = (new LocParser()).returnNormLcCall(num)

      // Author suffix:
      var author = flat.objectUri['dcterms:contributor:label'] &&
        flat.objectUri['dcterms:contributor:label'].length > 0
        ? flat.objectUri['dcterms:contributor:label'][0].label
        : ''
      var maxAuthorLength = 30
      results.idLccSort += exports.lpad(author.substr(0, maxAuthorLength), maxAuthorLength, ' ')

      // Work-created suffix:
      var created = flat.objectLiteral['dcterms:created'] &&
        flat.objectLiteral['dcterms:created'].length > 0
        ? flat.objectLiteral['dcterms:created'][0]
        : ''
      results.idLccSort += '.' + exports.lpad(created, 4)

      // Finally, uri suffix:
      results.idLccSort += '.' + exports.lpad(agentObject.uri, 10)

      console.log('Saving LCC :', num, 'Sort: ', results.idLccSort)
    })
  }

  if (flat.objectLiteral['classify:holdings']) {
  ...
  if (flat.objectLiteral['dcterms:created']) {
    // TODO: More flexible date parsing (e.g. "ca. 188-")
    var year = parseInt(flat.objectLiteral['dcterms:created'])
    if (!isNaN(year) && year >= 1970 && year < 2100) {
      var dateParsed = Date.parse(flat.objectLiteral['dcterms:created'])
      // FIXME: Handle bad dates like "2861" in a futureproof way:
      if (dateParsed) {
        results['created'] = dateParsed
        addResultYear('created', flat.objectLiteral['dcterms:created'])
      }
    }
  }
  ...
  config['predicatesResources'].filter(function (pred) { return pred.match(/^roles:/) }).forEach(function (pred) {
    if (flat.objectUri[pred]) {
      ...
      if (!results['contributor_packed']) results['contributor_packed'] = []
      results['contributor_packed'] = results['contributor_packed'].concat(flat.objectUri[`${pred}:label`].map((pair) => [pair.uri, pair.label].join(PACK_DELIM)))
